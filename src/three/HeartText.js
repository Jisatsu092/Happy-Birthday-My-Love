...
export class HeartText {
    constructor(scene) {
        ...
        this.config = {
            text: 'Only For You "Delfi Suryani"',
            size: 12,
            height: 0.05,
            bevelEnabled: false,
            bevelThickness: 0,
    ...
    renderText3D(newText) {
        ...
        lines.forEach((line, i) => {
            ...
            // simpan mesh pertama agar kompatibel dengan animasi lama
            if (i === 0) {
                this.textMesh = mesh;
            }
        });
        ...
        if (!this._textLight) {
            const textLight = new THREE.DirectionalLight(0xffffff, 2);
            textLight.position.set(0, 0, 100);
    ...
    setHeight(height) {
        this.config.height = Math.max(height, 0.01);
        this.renderText3D();
    }
    setColor(color) {
